...
use rafx::api::*;
use rafx::framework::{
    CookedShaderPackage, DescriptorSetBindings, FixedFunctionState, MaterialPass, ResourceContext,
...
fn run() -> RafxResult<()> {
    ...
    //
    let mut api = unsafe { RafxApi::new(&window, &Default::default())? };
    ...
    // clean shutdown.
    {
        ...
        let cooked_shaders_base_path =
            std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("cooked_shaders");
            ...
            *control_flow = ControlFlow::Poll;
            ...
            match event {
                ...
                Event::MainEventsCleared => {
                    ...
                    let wait_duration = if frame_duration < frame_time {
                        frame_time - frame_duration
                    } else {
                        Duration::from_millis(0)
                    };
                    *control_flow = ControlFlow::WaitUntil(frame_end + wait_duration);
                }
                Event::LoopDestroyed => {}
                _ => (),
            }
...
fn render(
    ...
    //
    let command_buffers = executor.execute_graph(PreparedRenderData::empty(), &graphics_queue)?;
